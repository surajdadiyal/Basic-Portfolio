...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms: Record<string, UniformDefinition> = useMemo(() => {
    ...
    return {
      u_colors: {
        value: colorsArray.map((c) => c.map((v: number) => v / 255)),
        type: "uniform3fv",
      },
      u_opacities: {
        value: opacities,
...
const ShaderMaterial = ({
  ...
  const shaderUniforms = useMemo(() => {
    const result: { [key: string]: THREE.IUniform } = {};

    for (const key in uniforms) {
      const u = uniforms[key];
      switch (u.type) {
        case "uniform1f":
          result[key] = { value: u.value as number };
          break;
        case "uniform1fv":
          result[key] = { value: u.value as number[] };
          break;
        case "uniform2f":
          result[key] = { value: new THREE.Vector2(...(u.value as number[])) };
          break;
        case "uniform3f":
          result[key] = { value: new THREE.Vector3(...(u.value as number[])) };
          break;
        case "uniform3fv":
          result[key] = {
